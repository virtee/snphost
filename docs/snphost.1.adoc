...
*snphost fetch ca*::
	usage: snphost fetch ca [ der, pem ] DIR-PATH [-e, --endorser] $ENDORSER

        This command fetches the host system's CA certificate chain and writes
        the encoded certificates to the directory at path specified directory (DIR-PATH). 
        Users must specify the desired certificate encoding format: either der or pem. 
        Optionally, users can specify which certificate endorser to fetch CA certificates for using the --endorser flag.
        The default is VCEK, but you can specify either VCEK or VLEK.

 options:
    -h, --help          Show a help message
    -e, --endorser      Choose to fetch the CA certificates for either the VLEK or VCEK. 
