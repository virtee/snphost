...
pub fn cmd(verify: VerifyCmd, quiet: bool) -> Result<()> {
    match verify {
        VerifyCmd::Certs(args) => certificate_chain::cmd(args, quiet),
        VerifyCmd::VlekHashstick(args) => vlek_hashstick::cmd(args, quiet),
    }
}
